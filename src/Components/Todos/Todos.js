          ...
          ? (
            ...
            <div className="FocusedTodo" key={focusedTodo.id} onClick={() => setIsComponentInFocus(true)}>
              ...
              {speedrun &&
                <label>
                  /
                  <input type="text" value={(focusedTodo.expectedTime < 10 ) ? "0"+focusedTodo.expectedTime : focusedTodo.expectedTime } onChange={(e) => handleEditExpectedTime(e, focusedTodo.id)} />
                  :00
                </label>
              }
              <svg className="actionable" width="20" height="12" viewBox="0 0 20 12" fill="none" xmlns="http://www.w3.org/2000/svg" >
                <path d="M0.721692 1.05223C1.12014 0.653774 1.76616 0.653773 2.16462 1.05223L11.2171 10.1047L9.77416 11.5476L0.721691 2.49515C0.323238 2.0967 0.323239 1.45068 0.721692 1.05223Z" fill="var(--secondary-text-color)"/>
                <path d="M18.8244 1.05061C19.2229 1.44906 19.2229 2.09508 18.8244 2.49353L9.77196 11.546L8.32904 10.1031L17.3815 1.05061C17.78 0.652156 18.426 0.652157 18.8244 1.05061Z" fill="var(--secondary-text-color)"/>
              </svg>
            </div>
          )
        }
            ...
            return (
              <form className="todo" key={todo.id} onSubmit={handleFinishEdit}>
                <label className="actionable">
                  ...
                  <svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg" style={(todo.id === focusedTodo.id)?{opacity: "0"}:{}}>
                    <path d="M4.35355 0.646446C4.15829 0.451184 3.84171 0.451184 3.64645 0.646446L0.464466 3.82843C0.269204 4.02369 0.269204 4.34027 0.464466 4.53553C0.659728 4.7308 0.976311 4.7308 1.17157 4.53553L4 1.70711L6.82843 4.53553C7.02369 4.7308 7.34027 4.7308 7.53553 4.53553C7.7308 4.34027 7.7308 4.02369 7.53553 3.82843L4.35355 0.646446ZM4.5 12L4.5 1H3.5L3.5 12H4.5Z" fill="var(--primary-text-color, blue)"/>
                  </svg>
                ...
                {speedrun &&
                  <label>
                    /
                    <input type="text" value={(todo.expectedTime < 10 ) ? "0"+todo.expectedTime : todo.expectedTime } onChange={(e) => handleEditExpectedTime(e, todo.id)}/>
                    :00
                  </label>
                }
                <input type="submit" style={{display: "none"}}/>
                <svg className="actionable" fill="var(--secondary-text-color)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="20px" height="20px" onClick={() => deleteTodo(todo.id)}>
